...
with col_f2:
    # Verifica se a foto existe na raiz
    if os.path.exists("rapha.jpeg"):
        ...
        <div style="width:180px; height:180px; border-radius:50%; 
             ...
             color:white; box-shadow:0 10px 40px rgba(59,130,246,0.3); 
             margin:0 auto 1.5rem auto; border:3px solid {PRIMARY};">
             RP
        </div>
        """, unsafe_allow_html=True)
...
with k1:
    st.markdown("""<div class="kpi-box"><div class="kpi-value">16+</div><div class="kpi-label">anos de experiência</div></div>""", unsafe_allow_html=True)
with k2:
    st.markdown("""<div class="kpi-box"><div class="kpi-value">70%</div><div class="kpi-label">redução operacional</div></div>""", unsafe_allow_html=True)
with k3:
    st.markdown("""<div class="kpi-box"><div class="kpi-value">213k</div><div class="kpi-label">registros processados</div></div>""", unsafe_allow_html=True)
with k4:
    st.markdown("""<div class="kpi-box"><div class="kpi-value">2h→15m</div><div class="kpi-label">análises reduzidas</div></div>""", unsafe_allow_html=True)
with k5:
    st.markdown("""<div class="kpi-box"><div class="kpi-value">R$50bi</div><div class="kpi-label">dados analisados</div></div>""", unsafe_allow_html=True)
...
for category, techs in tech_stack.items():
    st.markdown(f'<div class="stack-category">{category}</div>', unsafe_allow_html=True)
...
# ============================================================================
# PROJETOS - USANDO APENAS st.write E st.markdown
# ============================================================================
st.markdown("""
<div class="section-header">
...
# ============================================================================
# ANÁLISE INTERATIVA (SIMPLIFICADA)
# ============================================================================
st.markdown("""
<div class="section-header">
...
with tabs[0]:
    ...
    if len(df_f) > 0:
        ...
        with mk1: st.metric("Contratos", f"{len(df_f):,}".replace(",", "."))
        with mk2: st.metric("Valor Total", f"R$ {df_f['Valor'].sum()/1e6:.1f}M")
        with mk3: st.metric("Taxa Sucesso", f"{(df_f['Status']=='Renegociado').mean()*100:.1f}%")
        g1, g2 = st.columns(2)
        with g1:
            fig = px.pie(df_f, names="Região", values="Valor", hole=0.55, color_discrete_sequence=CHART_COLORS, template=PLOTLY_TEMPLATE)
            fig.update_layout(margin=dict(l=10,r=10,t=10,b=10), height=320, paper_bgcolor="rgba(0,0,0,0)", font=dict(color=TEXT))
            st.plotly_chart(fig, use_container_width=True)
        with g2:
            fig = px.bar(df_f.groupby("Faixa", observed=False)["Valor"].sum().reset_index(), x="Faixa", y="Valor", color_discrete_sequence=[CHART_COLORS[0]], template=PLOTLY_TEMPLATE)
            fig.update_layout(margin=dict(l=10,r=10,t=10,b=10), height=320, paper_bgcolor="rgba(0,0,0,0)", font=dict(color=TEXT), xaxis=dict(showgrid=False), yaxis=dict(showgrid=True, gridcolor="rgba(148,163,184,0.1)"))
            st.plotly_chart(fig, use_container_width=True)
    else:
...
with tabs[1]:
    ...
    for estado in estados:
        for comb in combustiveis:
            ...
            for mes in meses:
                dados.append({"Estado": estado, "Combustível": comb, "Mês": mes, "Preço": base + np.random.normal(0, 0.15)})
    df_anp = pd.DataFrame(dados)
    fc1, fc2 = st.columns(2)
    with fc1: est_sel = st.selectbox("Estado", estados, index=0)
    with fc2: comb_sel = st.selectbox("Combustível", combustiveis, index=0)
    df_f = df_anp[(df_anp["Estado"] == est_sel) & (df_anp["Combustível"] == comb_sel)]
    if len(df_f) > 0:
        mc1, mc2 = st.columns(2)
        with mc1: st.metric("Preço Atual", f"R$ {df_f[df_f['Mês']=='Jun']['Preço'].values[0]:.2f}")
        with mc2:
            variacao = ((df_f["Preço"].max() - df_f["Preço"].min()) / df_f["Preço"].min()) * 100
            st.metric("Variação Semestral", f"{variacao:.1f}%")
        fig = px.line(df_f, x="Mês", y="Preço", markers=True, color_discrete_sequence=[CHART_COLORS[0]], template=PLOTLY_TEMPLATE)
        fig.update_layout(title=f"{comb_sel} - {est_sel}", margin=dict(l=20,r=20,t=50,b=20), height=350, paper_bgcolor="rgba(0,0,0,0)", font=dict(color=TEXT), xaxis=dict(showgrid=False), yaxis=dict(showgrid=True, gridcolor="rgba(148,163,184,0.1)"))
        st.plotly_chart(fig, use_container_width=True)
    else:
        st.warning("Nenhum dado encontrado.")
...
with tabs[2]:
    ...
    fig = px.line(df_evo, x="Mês", y="Horas", color="Tipo", markers=True, color_discrete_sequence=[CHART_COLORS[3], CHART_COLORS[0]], template=PLOTLY_TEMPLATE)
    fig.update_layout(title="Evolução de Horas - Antes vs Após Automação", margin=dict(l=20,r=20,t=60,b=20), height=380, paper_bgcolor="rgba(0,0,0,0)", font=dict(color=TEXT), xaxis=dict(showgrid=False), yaxis=dict(showgrid=True, gridcolor="rgba(148,163,184,0.1)"))
    st.plotly_chart(fig, use_container_width=True)
    r1, r2, r3 = st.columns(3)
    with r1: st.metric("Horas Economizadas", "1.108 h", "+93% eficiência")
    with r2: st.metric("Custo Evitado", "R$ 185k", "vs. contratação")
    with r3: st.metric("Projetos Entregues", "24", "+60% vs. ano anterior")
