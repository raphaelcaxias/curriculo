...
def get_colors():
    if st.session_state.theme == "dark":
        return {
            ...
            "card_bg": "rgba(59,130,246,0.06)", "border": "rgba(59,130,246,0.2)",
        ...
        return {
            "primary": "#1D4ED8", "secondary": "#0EA5E9", "bg": "#F1F5F9",
            "text": "#0F172A", "text_muted": "#475569",
            "card_bg": "#FFFFFF", "border": "#E2E8F0",
...
def load_css():
    ...
    <style>
        ...
        * {{ margin: 0; padding: 0; box-sizing: border-box; }}
        ...
        .tech-badge {{ background: {c['tag_bg']}; border: 1px solid {c['tag_border']}; padding: 0.4rem 1rem; border-radius: 999px; font-size: 0.85rem; font-weight: 500; color: {c['text']}; display: inline-block; margin: 0.2rem; }}

        .section-header {{ text-align: center; margin: 3rem 0 2rem; }}
        .section-label {{ background: {c['primary_light']}; padding: 0.3rem 1.2rem; border-radius: 999px; font-size: 0.75rem; font-weight: 600; color: {c['primary']}; display: inline-block; }}
        .section-title {{ font-size: 2.2rem; font-weight: 700; margin-top: 0.5rem; color: {c['text']}; }}

        .kpi-box {{ background: {c['card_bg']}; border: 1px solid {c['border']}; border-radius: 16px; padding: 1.5rem 1rem; text-align: center; transition: 0.3s; box-shadow: {c['shadow']}; }}
        ...
        .timeline::before {{ content: ''; position: absolute; left: 30px; top: 0; bottom: 0; width: 2px; background: linear-gradient(to bottom, {c['primary']}, {c['secondary']}, transparent); }}
        .timeline-item {{ position: relative; padding-left: 80px; margin-bottom: 2rem; }}
        .timeline-dot {{ position: absolute; left: 22px; top: 5px; width: 16px; height: 16px; border-radius: 50%; background: {c['primary']}; border: 3px solid {c['bg']}; box-shadow: 0 0 0 3px {c['primary']}; }}
        .timeline-card {{ background: {c['card_bg']}; border: 1px solid {c['border']}; border-radius: 16px; padding: 1.5rem; transition: 0.3s; box-shadow: {c['shadow']}; }}
        ...
        .timeline-tag {{ font-size: 0.7rem; background: {c['tag_bg']}; border: 1px solid {c['tag_border']}; padding: 0.2rem 0.6rem; border-radius: 6px; color: {c['text']}; display: inline-block; margin: 0.2rem; }}
        .timeline-badge {{ background: {c['primary']}; color: white; font-size: 0.65rem; font-weight: 700; padding: 0.15rem 0.6rem; border-radius: 999px; margin-left: 0.5rem; display: inline-block; }}

        .stack-category {{ font-size: 0.9rem; font-weight: 600; color: {c['primary']}; margin: 1rem 0 0.5rem; }}
        .stack-chip {{ background: {c['tag_bg']}; border: 1px solid {c['tag_border']}; padding: 0.4rem 1rem; border-radius: 8px; font-size: 0.9rem; font-weight: 500; color: {c['text']}; display: inline-block; margin: 0.2rem; transition: 0.2s; }}
        .stack-chip:hover {{ background: {c['primary']}; color: white; transform: translateY(-2px); }}

        .testimonial-card {{ background: {c['card_bg']}; border: 1px solid {c['border']}; border-radius: 16px; padding: 1.5rem; text-align: center; box-shadow: {c['shadow']}; }}
        ...
        .cert-card {{ background: linear-gradient(135deg, {c['primary_light']}, {c['bg']}); border: 1px solid {c['primary']}; border-radius: 16px; padding: 1.5rem 2rem; text-align: center; box-shadow: {c['shadow']}; }}
        ...
        .footer {{ margin-top: 4rem; padding: 2.5rem 2rem; background: {c['card_bg']}; border: 1px solid {c['border']}; border-radius: 16px; text-align: center; box-shadow: {c['shadow']}; }}
        ...
        .footer-link {{ background: {c['tag_bg']}; border: 1px solid {c['tag_border']}; padding: 0.4rem 1rem; border-radius: 8px; color: {c['text']}; text-decoration: none; font-size: 0.9rem; display: inline-block; margin: 0.2rem; transition: 0.2s; }}
        ...
        .stTabs [data-baseweb="tab"] {{ background: {c['tag_bg']}; border: 1px solid {c['tag_border']}; border-radius: 8px; padding: 0.6rem 1.2rem; color: {c['text']}; font-weight: 500; }}
        .stTabs [aria-selected="true"] {{ background: {c['primary']} !important; color: white !important; border-color: {c['primary']} !important; }}
    </style>
    """, unsafe_allow_html=True)

# ============================================================================
# FUNÇÕES DE RENDERIZAÇÃO (TUDO EMBUTIDO)
# ============================================================================
...
def render_hero():
    ...
    with col2:
        if os.path.exists("rapha.jpeg"):
            ...
            st.image("assets/rapha.jpeg", width=160)
        else:
    ...
    <p class="hero-subtitle">Mais de <strong>16 anos</strong> transformando dados brutos em decisões estratégicas. Especialista em automação, governança de dados e criação de dashboards de alto impacto.</p>
    <div>
...
def render_kpis():
    ...
    """, unsafe_allow_html=True)
    kpis = [("16+", "anos de experiência"), ("70%", "redução operacional"), ("213k", "registros processados"), ("2h→15m", "tempo de análise"), ("R$50bi", "dados analisados")]
    cols = st.columns(5)
